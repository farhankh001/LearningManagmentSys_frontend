...
import { useEditQuizMutation } from '../../../app/api/quizAndAssignmentApi'
...
function MCQQuizEditForm() {
    ...
    const [editMCQQuiz, { isError, isLoading, isSuccess, error }] = useEditQuizMutation()
    ...
    useEffect(() => {
        if (isSuccess) {
            ...
            setTimeout(() => {
                navigate(`/lesson-settings/${quizId}`);
            }, 1000);
        }
    }, [isSuccess, reset]);
    ...
    const createQuizSubmitHandler = (data: CreateQuizMCQFormType) => {
        try {
            const quizMCQSubmtData = {
                quizId: quizId,
                questions: data.questions,
            }
            editMCQQuiz(quizMCQSubmtData)
        } catch (error) {
            console.log(error)
        }
    }
...
export default MCQQuizEditForm

import { Alert, Box,  Button,  LinearProgress,  List,  ListItem,  Typography, useTheme } from '@mui/material'
import TextInputField from '../../components/Forms/InputFields/TextInputField'
import { CreateCourseFormType, EditCourseFormType, editCourseSchema,} from '../../types/create_course.types'
...
import { CreateCourseSbumitType } from '../../app/api/createCourseApi'
...
import { CheckCircle, Gavel, Update } from '@mui/icons-material'
import DOMPurify from 'dompurify';
import { uploadToCloudinary } from '../../utils/uploadToCloundinary'
import { useEditExistingCourseMutation } from '../../app/api/teacherDashApis'
const instructions=[
...
function EditExistingCourse() {
    const [editCourse, { isLoading, isSuccess, isError, error }] = useEditExistingCourseMutation();
    const {handleSubmit,getValues}=useFormContext<EditCourseFormType>()
    const currentValuesCheck=getValues()
    console.log("curent value check"+JSON.stringify(currentValuesCheck))
    ...
    const createCourseSumit:SubmitHandler<EditCourseFormType>=async(data:EditCourseFormType)=>{
        try {
            const validatedData=editCourseSchema(categories).parse(data)
            ...
            if (validatedData.course_thumbnail instanceof File) {
              ...
              courseThumbnail=validatedData.course_thumbnail
            }
            ...
            // Call the mutation
            await editCourse(createCourseSubmitData).unwrap();
        } catch (error) {
